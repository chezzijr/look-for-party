...
from app.models import (
    User, UserCreate, CommitmentLevel, CommunicationStyle,
    Quest, QuestCreate, QuestCategory, LocationType, QuestVisibility,
    QuestApplication, QuestApplicationCreate,
    Party, PartyCreate, PartyStatus,
    PartyMember, PartyMemberCreate
)
...
class UserCreateFactory(factory.Factory):
    ...
    preferred_commitment_level = factory.Faker("random_element", elements=[e.value for e in CommitmentLevel])
    communication_style = factory.Faker("random_element", elements=[e.value for e in CommunicationStyle])
...
class QuestCreateFactory(factory.Factory):
    ...
    category = factory.Faker("random_element", elements=[e.value for e in QuestCategory])
    party_size_min = factory.Faker("random_int", min=1, max=3)
    party_size_max = factory.Faker("random_int", min=4, max=8)
    required_commitment = factory.Faker("random_element", elements=[e.value for e in CommitmentLevel])
    location_type = factory.Faker("random_element", elements=[e.value for e in LocationType])
    location_detail = factory.Faker("address")
    starts_at = factory.Faker("future_datetime", end_date="+30d")
    deadline = factory.LazyAttribute(lambda obj: obj.starts_at + timedelta(days=1) if obj.starts_at else None)
    estimated_duration = factory.Faker("random_element", elements=["1-2 hours", "Half day", "Full day", "Weekend", "1 week"])
    visibility = factory.Faker("random_element", elements=[e.value for e in QuestVisibility])
...
def create_party_member(
    ...
    **kwargs
    ...
    return crud.create_party_member(
        session=db,
        member_in=member_in,
        party_id=party_id
    )
...
def create_quest_with_applications(db: Session, num_applications: int = 3) -> tuple[Quest, list[QuestApplication]]:
...
def create_party_with_members(db: Session, num_members: int = 3) -> tuple[Party, list[PartyMember]]:

...
def test_read_tags(client: TestClient, db: Session) -> None:
    ...
    tag1 = create_test_tag(db, category=TagCategory.PROGRAMMING, name=unique_name1)
    tag2 = create_test_tag(db, category=TagCategory.FRAMEWORK, name=unique_name2)
...
def test_read_tags_with_filters(client: TestClient, db: Session) -> None:
    ...
    programming_tag = create_test_tag(db, category=TagCategory.PROGRAMMING, name=programming_tag_name)
    framework_tag = create_test_tag(db, category=TagCategory.FRAMEWORK, name=framework_tag_name)
    
    # Test category filter
    response = client.get(f"{settings.API_V1_STR}/tags/?category={TagCategory.PROGRAMMING.value}")
    assert response.status_code == 200
...
def test_read_popular_tags(client: TestClient, db: Session) -> None:
    ...
    if len(data["data"]) >= 2:
        first_tag = next((tag for tag in data["data"] if tag["id"] == str(tag1.id)), None)
        second_tag = next((tag for tag in data["data"] if tag["id"] == str(tag2.id)), None)
...
def test_get_tag_suggestions(client: TestClient, db: Session) -> None:
...
def test_create_tag_duplicate_name(
    ...
    # Create existing tag
    existing_tag = create_test_tag(db, name="Duplicate Tag")
...
def test_read_my_user_tags(
    ...
    user_tag_in1 = UserTagCreate(tag_id=tag1.id, proficiency_level=ProficiencyLevel.INTERMEDIATE)
...
def test_read_quest_tags(client: TestClient, db: Session) -> None:
    ...
    # Create quest tag
    quest_tag_in = QuestTagCreate(tag_id=tag.id, is_required=True, min_proficiency=ProficiencyLevel.ADVANCED)
    ...
    assert len(data["data"]) == 1
    assert data["data"][0]["tag_id"] == str(tag.id)
    assert data["data"][0]["is_required"] == True

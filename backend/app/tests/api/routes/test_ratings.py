...
def test_create_rating(
    ...
    crud.create_party_member(
        session=db,
        member_in=PartyMemberCreate(user_id=current_user_id, role=PartyMemberRole.OWNER),
        party_id=party.id
    )
    crud.create_party_member(
        session=db,
...
def test_create_rating_party_not_completed(
    ...
    assert "Can only rate members when party is completed or archived" in response.json()["detail"]
...
def test_create_rating_duplicate(
    ...
    assert response.status_code == 400
    assert "You have already rated this user for this party" in response.json()["detail"]
...
def test_read_my_given_ratings(
    ...
    for rating in data["data"]:
        assert rating["rater_id"] == str(current_user_id)  # All ratings should be by current user
        if rating["party_id"] == str(party.id) and rating["rated_user_id"] == str(rated_user.id):
...
def test_read_user_received_ratings(client: TestClient, db: Session) -> None:
    ...
    response = client.get(f"{settings.API_V1_STR}/ratings/users/{rated_user.id}/received")
    assert response.status_code == 200
...
def test_read_user_rating_summary(client: TestClient, db: Session) -> None:
    ...
    response = client.get(f"{settings.API_V1_STR}/ratings/users/{rated_user.id}/summary")
    assert response.status_code == 200
...
def test_update_rating(
    ...
    rating = crud.create_rating(session=db, rating_in=rating_in, rater_id=current_user_id)

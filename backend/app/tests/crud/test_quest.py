...
def test_get_quests(db: Session) -> None:
    ...
    quest1 = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator1.id)
    quest2 = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator2.id)
...
def test_get_quests_with_status_filter(db: Session) -> None:
    ...
    quest_recruiting = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator.id)
    
    # Create completed quest
    quest_completed_data = QuestFactory()
    quest_completed = crud.create_quest(session=db, quest_in=quest_completed_data, creator_id=creator.id)
    crud.update_quest(session=db, db_quest=quest_completed, quest_in=QuestUpdate(status=QuestStatus.COMPLETED))
...
def test_get_quests_by_creator(db: Session) -> None:
    ...
    quest1 = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator1.id)
    quest2 = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator1.id)
    quest3 = crud.create_quest(session=db, quest_in=QuestFactory(), creator_id=creator2.id)

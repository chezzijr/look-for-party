...
def test_get_tags(db: Session) -> None:
    ...
    tag1 = create_test_tag(db, category=TagCategory.PROGRAMMING, status=TagStatus.SYSTEM)
    tag2 = create_test_tag(db, category=TagCategory.FRAMEWORK, status=TagStatus.APPROVED)
    tag3 = create_test_tag(db, category=TagCategory.PROGRAMMING, status=TagStatus.PENDING)
    ...
    programming_tags = crud.get_tags(session=db, category=TagCategory.PROGRAMMING, limit=1000)
    ...
    # Test search
    search_term = tag1.name[:8].upper()  # Use first 8 chars and make uppercase to match TEST_ONLY
...
def test_create_user_tag(db: Session) -> None:
    ...
    user_tag = crud.create_user_tag(session=db, user_tag_in=user_tag_in, user_id=user.id)
...
def test_get_user_tags(db: Session) -> None:
    ...
    # Create user tags
    user_tag_in1 = UserTagCreate(tag_id=tag1.id, proficiency_level=ProficiencyLevel.BEGINNER)
    user_tag_in2 = UserTagCreate(tag_id=tag2.id, proficiency_level=ProficiencyLevel.EXPERT)
...
def test_get_user_tag(db: Session) -> None:
    ...
    user_tag_in = UserTagCreate(tag_id=tag.id, proficiency_level=ProficiencyLevel.INTERMEDIATE)
    created_user_tag = crud.create_user_tag(session=db, user_tag_in=user_tag_in, user_id=user.id)
...
def test_update_user_tag(db: Session) -> None:
    ...
    user_tag_in = UserTagCreate(tag_id=tag.id, proficiency_level=ProficiencyLevel.BEGINNER)
    user_tag = crud.create_user_tag(session=db, user_tag_in=user_tag_in, user_id=user.id)
    ...
    updated_user_tag = crud.update_user_tag(session=db, db_user_tag=user_tag, user_tag_in=user_tag_update)
    
    assert updated_user_tag.proficiency_level == ProficiencyLevel.ADVANCED
    assert updated_user_tag.is_primary == True
...
def test_create_quest_tag(db: Session) -> None:
    ...
    quest_tag = crud.create_quest_tag(session=db, quest_tag_in=quest_tag_in, quest_id=quest.id)
    
    assert quest_tag.quest_id == quest.id
    assert quest_tag.tag_id == tag.id
    assert quest_tag.is_required == True
...
def test_get_quest_tags(db: Session) -> None:
    ...
    # Create quest tags
    quest_tag_in1 = QuestTagCreate(tag_id=tag1.id, is_required=True, min_proficiency=ProficiencyLevel.INTERMEDIATE)
    quest_tag_in2 = QuestTagCreate(tag_id=tag2.id, is_required=False, min_proficiency=ProficiencyLevel.BEGINNER)
...
def test_get_quest_tag(db: Session) -> None:
    ...
    quest_tag_in = QuestTagCreate(tag_id=tag.id, is_required=True, min_proficiency=ProficiencyLevel.INTERMEDIATE)
    created_quest_tag = crud.create_quest_tag(session=db, quest_tag_in=quest_tag_in, quest_id=quest.id)
    
    retrieved_quest_tag = crud.get_quest_tag(session=db, quest_id=quest.id, tag_id=tag.id)
    
    assert retrieved_quest_tag
    assert retrieved_quest_tag.quest_id == quest.id
    assert retrieved_quest_tag.tag_id == tag.id
    assert retrieved_quest_tag.is_required == True
...
def test_update_quest_tag(db: Session) -> None:
    ...
    quest_tag_in = QuestTagCreate(tag_id=tag.id, is_required=False, min_proficiency=ProficiencyLevel.BEGINNER)
    quest_tag = crud.create_quest_tag(session=db, quest_tag_in=quest_tag_in, quest_id=quest.id)
    ...
    updated_quest_tag = crud.update_quest_tag(session=db, db_quest_tag=quest_tag, quest_tag_in=quest_tag_update)
    
    assert updated_quest_tag.is_required == True

...
def apply_to_quest(
    *,
    ...
    if quest.status != QuestStatus.RECRUITING:
        raise HTTPException(status_code=400, detail="Quest is not accepting applications")
    ...
    for app in existing_applications:
        if app.quest_id == quest_id and app.status in [ApplicationStatus.PENDING, ApplicationStatus.APPROVED]:
...
def read_application(
    ...
    """
    application = crud.get_quest_application(session=session, application_id=application_id)
    if not application:
    ...
    if (application.applicant_id != current_user.id and 
        quest and quest.creator_id != current_user.id and 
    ...
    *,
    ...
    if application_in.status is not None:
        if not is_creator and not current_user.is_superuser:
            raise HTTPException(status_code=403, detail="Only quest creator can change application status")
        
        # Can only change from pending
        if application.status != ApplicationStatus.PENDING:
            raise HTTPException(status_code=400, detail="Can only approve/reject pending applications")
    ...
    if (application_in.message is not None or application_in.proposed_role is not None):
        if not is_applicant and not current_user.is_superuser:
            raise HTTPException(status_code=403, detail="Only applicant can edit application details")

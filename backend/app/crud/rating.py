...
def create_rating(*, session: Session, rating_in: RatingCreate, rater_id: uuid.UUID) -> Rating:
...
def get_party_ratings(*, session: Session, party_id: uuid.UUID) -> list[Rating]:
    ...
    statement = select(Rating).where(Rating.party_id == party_id).order_by(Rating.created_at.desc())
    return list(session.exec(statement).all())


def get_user_received_ratings(*, session: Session, user_id: uuid.UUID) -> list[Rating]:
    """Get all ratings received by a user."""
    statement = select(Rating).where(Rating.rated_user_id == user_id).order_by(Rating.created_at.desc())
    return list(session.exec(statement).all())


def get_user_given_ratings(*, session: Session, user_id: uuid.UUID) -> list[Rating]:
    """Get all ratings given by a user."""
    statement = select(Rating).where(Rating.rater_id == user_id).order_by(Rating.created_at.desc())
    return list(session.exec(statement).all())


def get_rating_between_users(
    *, session: Session, party_id: uuid.UUID, rater_id: uuid.UUID, rated_user_id: uuid.UUID
) -> Rating | None:
...
def update_rating(*, session: Session, db_rating: Rating, rating_in: RatingUpdate) -> Rating:
    """Update a rating."""
...
def get_user_rating_summary(*, session: Session, user_id: uuid.UUID) -> UserRatingSummary:
    ...
    positive_feedback_stats = select(
        ...
        func.sum(case((Rating.would_collaborate_again == True, 1), else_=0)).label("positive_count")
    ).where(Rating.rated_user_id == user_id)
    ...
    if feedback_result and feedback_result.total_count > 0:
        positive_percentage = (feedback_result.positive_count or 0) / feedback_result.total_count * 100
...
def get_ratable_users_for_party(*, session: Session, party_id: uuid.UUID, current_user_id: uuid.UUID) -> list[User]:
    ...
    statement = select(User).join(PartyMember).where(
    ...
    already_rated_users = session.exec(
        select(Rating.rated_user_id).where(
            Rating.party_id == party_id,
            Rating.rater_id == current_user_id
        )
    ).all()
...
def can_user_rate_party(*, session: Session, party_id: uuid.UUID, user_id: uuid.UUID) -> bool:

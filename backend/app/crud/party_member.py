...
def create_party_member(*, session: Session, member_in: PartyMemberCreate, party_id: uuid.UUID) -> PartyMember:
    db_member = PartyMember.model_validate(
        member_in,
...
def get_party_members(*, session: Session, party_id: uuid.UUID, active_only: bool = True) -> list[PartyMember]:
    statement = select(PartyMember).where(PartyMember.party_id == party_id)
    if active_only:
        statement = statement.where(PartyMember.is_active == True)
    statement = statement.order_by(PartyMember.joined_at)
    return list(session.exec(statement).all())


def get_user_party_memberships(*, session: Session, user_id: uuid.UUID, active_only: bool = True) -> list[PartyMember]:
...
def update_party_member(*, session: Session, db_member: PartyMember, member_in: PartyMemberUpdate) -> PartyMember:
